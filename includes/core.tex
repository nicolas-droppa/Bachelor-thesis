...
\section {Python}
Python je interpretovaný objektovo orientovaný, vysokoúrovňový programovací jazyk s dynamickou śemantikou. 
O vznik tohoto programovacieho jazyka sa zaslúžil Guido van Rossum. Python bol vydaný v roku 1991 ~\cite{teradata_python}. 
Jeho vysokoúrovňové vstavané dátové štruktúry v kombinácii s dynamickým písaníma a dynamickým viazaním ho 
robia veľmi atraktívnym pre rýchly vývoj aplikácií, ako aj pre použitie ako skriptovací alebo lepiaci jazyk 
na prepojenie existujúcich komponentov ~\cite{python_org}. 

Python je jedným z najznámejších programovacích jazykov práve kvôli jeho jednoduchej a zrozumiteľnej syntaxi, 
bloky kódu sú definované pomocou odsádzania bez potreby používania zložených zátvoriek. 

\subsection{Výhody jazyka python}
Medzi najväčšie výhody jazyka patrí čitateľnosť jeho kódu, jeho podpora rôznych štýlov programovania, vrátane 
procedurálneho, objektovo-orientovaného a funkcionálneho programovania. Vďaka obľúbenosti tohoto jazyka medzi 
programátormi po celom svete, tento jazyk disponuje množstvom dokumentácie a tutoriálov.

\subsection{Nevýhody jazyka python}
Python je interpretovaný jazyk, to znamená, že je pomalší ako kompilované jazyky ako sú C, C++ alebo Pascal. 
Jednou z najväčších nevýhod je spotreba pamäte. Dynamická povaha premenných a objektov jazyka spôsobuje určitú 
réžiu z hľadiska pamäte. Keďže jazyk musí byť pripravený na neustále sa meniace typy údajov, môže alokovať viac 
pamäte, ako je nevyhnutne potrebné ~\cite{codewithc_python_memory}. 
