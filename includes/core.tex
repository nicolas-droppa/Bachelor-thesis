...
\textbf{1. Elektronické}, kde sa využívajú moderné senzory spolu so 
...
Práve tieto disciplíny boli zvolené pre poučenie čitateľa na základe toho, že majú odlišné terče a
...
\subsection{Voľba hodnotiacich parametrov pre aplikáciu}
...
Pri fotení terčov je potrebné dodržiavať následné pravidlá. Terč musí byť na obrázku úplne viditeľný - nesmú chýbať žiadne rohy ani okraje. Papier, na ktorom 
...
\begin{itemize}
  ...
  \item \textbf{Výpočet metrík} - najlepší a najhorší výstrel, priemerné skóre, mean radius, radius variance, consistency (MAD), windage, elevation, max 
  spread a historické štatistiky pre sledovanie progresu.
...
\subsection{Diagramy}
V tejto kapitole budeme vytvárať diagramy pre popísanie funcionality našej aplikácie. Pre najzákladnejší opis toho, čo aplikácia dovoluje
navrhneme diagram prípadov použitia. Následne budeme vytvárať entitno relačný diagram, ktorý bude opisovať to aké stĺpce a polia bude
databáza obsahovať.

\subsubsection{Diagram prípadov použitia}
Na obr.~\ref{fig:obrazok3} je znázornený diagram prípadov použitia, ktorý popisuje hlavné interakcie medzi používateľom (strelec) a 
systémom aplikácie. Primárnym aktérom je \emph{Používateľ}, ktorý vykonáva nasledovné prípady použitia:

\begin{itemize}
  \item \textbf{Nahratie fotografie terča} - vstupný krok, pri ktorom používateľ nahraje snímku terča z galérie. Tento prípad použitia \emph{zahŕňa} (include) 
  \emph{Zobrazenie vyhodnotenia} a \emph{Detekciu zásahov}.
  \item \textbf{Detekcia zásahov} - automatický výpočet polohy a skóre zásahov. Ak detekcia nevyhovuje, rozširuje sa (extend) o \emph{Manuálnu úpravu detekcie} 
  alebo \emph{Vyklikanie výstrelov}, ktoré umožňujú používateľovi doladiť alebo označiť chýbajúce zásahy.
  \item \textbf{Zobrazenie vyhodnotenia} - okamžitá vizualizácia výsledkov a metrík po spracovaní snímky.
  \item \textbf{Prehľad výkonov} - samostatný prípad použitia umožňujúci pristupovať k histórii všetkých predchádzajúcich sedení a štatistík.
\end{itemize}

Diagram využíva spojky \emph{include} na označenie povinných krokov, ktoré sa vždy vykonajú (napr. zobrazenie výsledkov po nahratí fotografie) a spojky 
\emph{extend} pre voliteľné, situáciu-podmienené akcie (napr. manuálna korekcia detekcie zásahov). Takto rozdelené logické bloky uľahčujú pochopenie, 
ktoré funkcie sú nevyhnutné pre základný tok, a ktoré sa aktivujú iba v prípade potreby doplnenia či opravy automatizovaných procesov.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{img/user-case.pdf}
  \caption{Diagram prípadov použitia pre aplikáciu}
  \label{fig:obrazok3}
\end{figure}

\subsubsection{ER diagram}

Na obr.~\ref{fig:er_diagram} je znázornený ER diagram, ktorý obsahuje entity, ich atribúty a vzťahy v navrhovanej aplikácii na hodnotenie strelby
na terč. Diagram pozostáva z dvoch hlavných entít \emph{User} a \emph{Session\_Stats}, medzi ktorými je definovaný many-to-many vzťah.

\begin{itemize}
  \item \textbf{User} (entita) - predstavuje registrovaného používateľa aplikácie.  
    Atribúty:  
    \begin{itemize}
      \item \texttt{id (PK)} - primárny kľúč  
      \item \texttt{name} - meno používateľa  
      \item \texttt{email} - e-mailová adresa (index)  
      \item \texttt{password} - heslo (hash)  
      \item \texttt{created\_at} - dátum registrácie  
    \end{itemize}

  \item \textbf{Session\_Stats} (entita) – predstavuje jednu streleckú sériu so súhrnnými metrikami.  
    Atribúty:  
    \begin{itemize}
      \item \texttt{id (PK)} - primárny kľúč  
      \item \texttt{recorded\_at} - dátum a čas záznamu  
      \item \texttt{total\_shots}, \texttt{best\_score}, \texttt{worst\_score}, \texttt{average\_score}  
      \item \texttt{mean\_radius\_mm}, \texttt{variance\_mm2}, \texttt{consistency\_mm}  
      \item \texttt{elevation\_mm}, \texttt{windage\_mm}, \texttt{max\_spread\_mm}  
    \end{itemize}
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{img/er-diagram.pdf}
  \caption{ER diagram - entity \emph{User}, \emph{Session\_Stats}}
  \label{fig:er_diagram}
\end{figure}


\section{Implementácia riešenia}
Po navrhnutí riešenia celej práce a vyčlenení pravidiel a podmienok pre správne fungovanie sme začali s implementáciou. Táto kapitola bude rozdelená
do niekoľkých častí, ako prvú veľkú časť sme zvolili nahratie obrázku terča pre vyhodnotenie. Pre následné umožnenie používateľovi vyklikať výsledky
pre vyhodnotenie sme najprv museli zabezpečiť to, že obrázku nahratému do aplikácie bude transformovaná perspektíva.

\subsection{Nahratie obrázku a transformácia perspektívy}
Prvým krokom našej práce bolo umožniť používateľovi vybrať fotografiu terča z miestneho úložiska. V používateľskom rozhraní je to realizované pomocou HTML prvku
\verb|<input type="file">|, na ktorý je napojený ``change'' event listener. Po výbere súboru sa v JavaScripte vytvorí objekt \verb|Image|, pričom sa 
jeho \verb|src| nastaví na lokálnu URL súboru.

Po nahratí obrázku budeme tento obrázok vykreslovať do \verb|<canvas>| elementu s identifikátorom \verb|warpCanvas|. Je však potrebné zabezpečiť aby
sa pri rôznych veľkostiach obrázku ``nerozbilo'' rozloženie našej webovej aplikácie. Následne v aplikácii vyčistíme predošlý obsah a uložíme pixelové
data do 2D kontextu (\verb|CanvasRenderingContext2D|).

\subsubsection{Detekcia rohov papiera terča – podrobný popis}
...
\paragraph{1. Načítanie a prvotné zobrazenie obrazu}  
Prvým krokom je načítanie vybranej fotografie do OpenCV matice pomocou \verb|cv.imread()|. Bez tohto volania by sme nemohli použiť žiadne ďalšie spracovanie:

\begin{lstlisting}[
  style=code-listing,
  language=JavaScript,
  caption={Načítanie obrázku do OpenCV matice},
  label={lst:imread}
]
...
\paragraph{2. Konverzia do medzipodôb}  
Aby sme mohli presne vyseparovať papier od pozadia, transformujeme farbu do viacerých reprezentácií – odtieň šedej pre binárne prahovanie a HSV pre farebné maskovanie. Obe konverzie používame pre robustnejšiu detekciu:

\begin{lstlisting}[
  caption={Konverzia na šedú a HSV pred maskou},
  label={lst:color-conversion},
  language=JavaScript,
  style=code-listing
]
...
\paragraph{3. Filterovanie a reťazenie rozostrení}  
Priame prahovanie často trpí šumom alebo drobnými škvrnami na papieri. Preto na masku aplikujeme v postupnosti:
\begin{itemize}
  \item \verb|applyMedianBlur()| – odstraňuje drobné salt-and-pepper artefakty,  
  \item \verb|applyGaussianBlur()| – uhladzuje hrany kontúr,  
  \item \verb|applyDefaultBlur()|  – zjemňuje prechody a pripravuje na spoľahlivejšiu detekciu kontúr.
\end{itemize}

\begin{lstlisting}[
  caption={Reťazenie blur operácií pre elimináciu šumu},
  label={lst:blur-chain},
  language=JavaScript,
  style=code-listing
]
...
\paragraph{4. Vyhľadanie a výber najväčšej kontúry}  
Z rozostrenej masky zavoláme \verb|findContours()| a vyberieme tú, ktorá má najväčšiu plochu (\verb|getLargestContour()|). Táto kontúra najpravdepodobnejšie zodpovedá obrysu papierovej plochy:

\begin{lstlisting}[
  caption={Detekcia a výber najväčšej kontúry},
  label={lst:contour-detection},
  language=JavaScript,
  style=code-listing
]
...
Funkcia \verb|cv.imread| načíta obsah plátna do objektu typu \verb|cv.Mat|, s ktorým už dokážeme pracovať ako s maticou v OpenCV.js.

Pre odstránenie perspektívneho skreslenia implementujeme v module \verb|utils/imageProcessing.js| funkciu

\begin{verbatim}
warpPerspective(srcMat, corners)
\end{verbatim}

kde \verb|corners| je pole štyroch bodov (získaných buď automaticky alebo manuálne kliknutím), definujúcich rohy terča. Funkcia vypočíta transformačnú maticu (homografiu)
a aplikuje ju na \verb|srcMat|, čím vznikne nový \verb|cv.Mat| narovnaného výrezu terča. Výsledok vykreslíme späť na plátno pomocou

\begin{verbatim}
cv.imshow(warpCanvas, warpedMat);
\end{verbatim}

a odstránime dočasné matice volaniami \verb|srcMat.delete()| a \verb|warpedMat.delete()|, aby sme predišli úniku pamäte.
...
Sú to napríklad RTG (rontgenové žiarenie),
